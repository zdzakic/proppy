...
type EditBlockModalProps = {
  ...
  isSaving: boolean;
  error: string | null;
  onSave: (payload: UpdateBlockPayload) => Promise<void>;
  onClose: () => void;
};
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-dashboard-sidebar/40 p-3 sm:items-center sm:p-4">
      <div className="w-full max-w-xl rounded-xl border border-dashboard-border bg-dashboard-surface p-4 shadow-premium sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-dashboard-text">Update Block</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <div className="space-y-1">
            <label htmlFor="edit-block-name" className="text-sm text-dashboard-muted">
              Block Name
            </label>
            <input
              id="edit-block-name"
